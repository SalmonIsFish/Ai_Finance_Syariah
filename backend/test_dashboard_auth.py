...
def auth_client(monkeypatch):
    users_json = json.dumps([
        {"username": "project_owner", "password_hash": auth.hash_password("owner_pass", iterations=100), "role": "admin"},
        {"username": "other_user", "password_hash": auth.hash_password("other_pass", iterations=100), "role": "admin"}
    ])
...
def test_dashboard_endpoints_require_owner_auth(auth_client):
    ...
    owner_headers = {"Authorization": f"Basic {base64.b64encode(b'project_owner:owner_pass').decode()}"}
    other_headers = {"Authorization": f"Basic {base64.b64encode(b'other_user:other_pass').decode()}"}
