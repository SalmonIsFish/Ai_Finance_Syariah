...
import json
import base64
from backend.local_api import app
    ...
    users_json = json.dumps(
        [
            ...
            },
            {
                ...
                "password_hash": auth.hash_password("review_pass", iterations=1000),
                "role": "reviewer",
            },
        ]
    )
...
def test_p3_api_auth_requirements(monkeypatch):
    ...
    # Unauthenticated
    res = client.post("/p3/portfolios/1/proposals", json={"ticker": "AAPL", "side": "BUY"})
    assert res.status_code == 401
    assert "Not authenticated" in res.json().get(
        "detail", ""
    ) or "Incorrect username or password" in res.json().get("detail", "")

    # Forged actor without auth
    res = client.post(
        "/p3/portfolios/1/proposals", json={"ticker": "AAPL", "side": "BUY", "actor": "admin_user"}
    )
    assert res.status_code == 401

    # Authenticated but bad password
    res = client.post(
        "/p3/portfolios/1/proposals",
        json={"ticker": "AAPL", "side": "BUY"},
        headers=get_auth_headers("admin_user", "wrong"),
    )
    assert res.status_code == 401
    ...
    res = client.post(
        "/p3/portfolios/1/proposals",
        json={"ticker": "AAPL", "side": "BUY"},
        headers=get_auth_headers("review_user", "review_pass"),
    )
    ...
    res = client.post(
        f"/p3/portfolios/1/orders/{order_id}/approve",
        json={},
        headers=get_auth_headers("review_user", "review_pass"),
    )
    ...
    res = client.post(
        f"/p3/portfolios/1/orders/{order_id}/execute",
        json={},
        headers=get_auth_headers("review_user", "review_pass"),
    )
    ...
    res = client.post(
        f"/p3/portfolios/1/orders/{order_id}/execute",
        json={},
        headers=get_auth_headers("admin_user", "admin_pass"),
    )
    assert res.status_code == 200
