...
def authoritative_revalidation(
    ...
    try:
        ...
        if not bars:
            return {"status": "BLOCKED", "reason": "market_data_unavailable", "details": {"ticker": ticker}}
    except Exception:
        return {"status": "BLOCKED", "reason": "market_data_unavailable", "details": {"ticker": ticker}}
    ...
    from backend.pure_risk import RiskState, RiskPolicy, calculate_target_quantity, LossPerUnitRequiredError
    ...
    for p in positions:
        try:
            ...
            if not bars:
                return {"status": "BLOCKED", "reason": "portfolio_valuation_unavailable", "details": {"ticker": p["ticker"]}}
        except Exception:
            return {"status": "BLOCKED", "reason": "portfolio_valuation_unavailable", "details": {"ticker": p["ticker"]}}
    ...
    try:
        maximum_quantity = calculate_target_quantity(state, ticker, current_price, sector, policy, side, loss_per_unit=None)
