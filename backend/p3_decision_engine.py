...
def authoritative_revalidation(
    ...
    if shariah.get("status") != "PASS":
        return {"status": "BLOCKED", "reason": f"shariah_gate_failed: {shariah.get('status')}", "details": shariah}
    ...
    bars, source = yahoo_finance.fetch_eod_prices(
        ...
        allow_stale_cache=False if for_execution else True
    )
    if not bars:
        return {"status": "BLOCKED", "reason": "market_data_unavailable", "details": {}}
    ...
    total_exposure = sum(p["quantity"] * current_price for p in positions)
    ...
    max_sector_exposure_pct = limits.get("max_sector_exposure_pct", 10.0)
    max_loss_per_trade_pct = limits.get("max_loss_per_trade_pct", 0.5)
    
    # Sizing constraints (in terms of allowed quantity)
    # 1. Cash limit
    cash_limit_qty = cash / current_price if side == "BUY" else float('inf')
    ...
    existing_exposure = existing_qty * current_price
    max_position_exposure = actual_equity * (max_position_pct / 100.0)
    allowed_position_add_exposure = max(0.0, max_position_exposure - existing_exposure)
    position_limit_qty = allowed_position_add_exposure / current_price if side == "BUY" else float('inf')
    
    # 3. Total exposure limit
    max_total_exposure = actual_equity * (max_total_exposure_pct / 100.0)
    allowed_total_add_exposure = max(0.0, max_total_exposure - total_exposure)
    total_exposure_qty = allowed_total_add_exposure / current_price if side == "BUY" else float('inf')
    
    # 4. Sector limit
    sector = shariah.get("sector") or "UNKNOWN"
    sector_exposure = sum(p["quantity"] * current_price for p in positions if p["sector"] == sector)
    max_sector_exposure = actual_equity * (max_sector_exposure_pct / 100.0)
    allowed_sector_add_exposure = max(0.0, max_sector_exposure - sector_exposure)
    sector_limit_qty = allowed_sector_add_exposure / current_price if side == "BUY" else float('inf')
    
    # 5. 0.5% max loss per trade rule (worst-case notional as loss)
    max_loss_exposure = actual_equity * (max_loss_per_trade_pct / 100.0)
    per_trade_loss_qty = max_loss_exposure / current_price if side == "BUY" else float('inf')
    
    if side == "SELL":
        maximum_quantity = existing_qty
    else:
    ...
    # Floor at 0, no fractional for now (or allow fractional, but lets stick to precision)
    maximum_quantity = max(0.0, maximum_quantity)
...
def propose_order(connection: sqlite3.Connection, portfolio_id: int, ticker: str, side: str, actor: str = "SYSTEM") -> dict:
    # Proposal time validation
    validation = authoritative_revalidation(connection, portfolio_id, ticker, side, requested_price=0.0)
    ...
    cursor = connection.execute(
        ...
        """,
        (
            portfolio_id, ticker, side, qty, price, utc_now(), actor,
            shariah.get("publication_id"), shariah.get("status")
        )
    )
    connection.commit()
...
def approve_order(connection: sqlite3.Connection, order_id: int, actor: str) -> dict:
    ...
    validation = authoritative_revalidation(connection, order["portfolio_id"], order["ticker"], order["side"], requested_price=order["requested_price"], proposed_quantity=order["quantity"])
    if validation["status"] != "PASS":
        # Reject the order
        p3_portfolio_engine.update_order_status(connection, order_id, "PROPOSED", "REJECTED", {"rejection_reason": validation["reason"], "actor": actor})
        raise ValueError(f"Approval validation failed: {validation['reason']}")
    
    # Transition
    return p3_portfolio_engine.update_order_status(connection, order_id, "PROPOSED", "APPROVED", {"actor": actor})
...
def execute_order(connection: sqlite3.Connection, order_id: int, actor: str) -> dict:
    ...
    try:
        ...
        # Update cash
        portfolio = p3_portfolio_engine.get_portfolio(connection, portfolio_id)
        new_cash = portfolio["current_cash"] - notional if side == "BUY" else portfolio["current_cash"] + notional
        if new_cash < 0:
            raise ValueError("Insufficient cash during execution")
            
        connection.execute("UPDATE p3_portfolios SET current_cash = ?, updated_at = ? WHERE id = ?", (new_cash, utc_now(), portfolio_id))
        ...
        # Update position
        existing_pos = connection.execute("SELECT * FROM p3_portfolio_positions WHERE portfolio_id = ? AND ticker = ?", (portfolio_id, ticker)).fetchone()
        if side == "BUY":
            if existing_pos:
                old_qty = existing_pos["quantity"]
                old_cost = existing_pos["average_cost"]
                new_qty = old_qty + qty
                new_avg_cost = ((old_qty * old_cost) + (qty * exec_price)) / new_qty if new_qty > 0 else 0
                connection.execute(
                ...
                connection.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, 0, 0, ?, ?)
                    """,
                    (portfolio_id, ticker, qty, exec_price, qty * exec_price, shariah.get("sector"), utc_now())
                )
        else: # SELL
        ...
        order = p3_portfolio_engine.update_order_status(
            connection, order_id, "APPROVED", "EXECUTED", 
